...
<div class="container mx-auto py-6 px-6 bg-slate-50 min-h-screen">
    ...
    <div class="bg-white border border-slate-200 rounded-xl px-5 py-4 mb-6 shadow-sm flex items-center justify-between">
        ...
        <div>
            <a href="<?php echo e(route('admin.Adventures.create')); ?>" class="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded-lg shadow-sm shadow-blue-500/10 transition-all flex items-center gap-2 text-sm">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse" id="myTable">
                ...
                <tbody>
                    <?php $__empty_1 = true; $__currentLoopData = $Adventures; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $deposit): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-50 border-b border-slate-100 transition-all">
                            <td class="p-4 text-sm font-semibold text-slate-500">
                                <?php echo e(($Adventures->currentPage() - 1) * $Adventures->perPage() + $loop->iteration); ?>

                            </td>
                            <td class="px-5 py-4">
                                <span class="font-bold text-slate-800 text-sm block"><?php echo e($deposit->plan); ?></span>
                                <span class="text-xs text-slate-400 mt-0.5">Name: <?php echo e($deposit->name); ?></span>
                            </td>
                            <td class="px-4 py-4 text-sm">
                                <?php if(!empty($deposit->percentage_range)): ?>
                                    <span class="bg-blue-50 text-blue-700 text-xs font-bold px-2 py-0.5 rounded border border-blue-100 block w-max"><?php echo e($deposit->percentage_range); ?></span>
                                <?php else: ?>
                                    <span class="font-bold text-indigo-600 block"><?php echo e($deposit->percentage); ?>%</span>
                                <?php endif; ?>
                                <span class="text-xs text-slate-400 mt-1 block">Base Rate: <?php echo e($deposit->percentage); ?>%</span>
                            </td>
                            <td class="px-5 py-4 text-sm">
                                <span class="font-semibold text-slate-700 block">$<?php echo e(number_format($deposit->min_amount, 0)); ?> - $<?php echo e(number_format($deposit->max_amount, 0)); ?></span>
                                <span class="text-xs text-slate-400 mt-0.5 block">Currency: <?php echo e($deposit->currency); ?></span>
                            </td>
                            <td class="px-4 py-4 text-sm">
                                <span class="font-semibold text-slate-600 block">$<?php echo e(number_format($deposit->current_price, 4)); ?></span>
                            ...
                            <td class="px-4 py-4 text-sm text-center">
                                ...
                                <?php if($deposit->payments()->count() > 0): ?>
                                    <span class="bg-emerald-100 text-emerald-800 text-xs font-bold px-2.5 py-1 rounded-full"><i class="fas fa-users mr-1"></i><?php echo e($deposit->payments()->count()); ?></span>
                                <?php else: ?>
                            ...
                            <td class="px-5 py-4 text-sm text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a href="<?php echo e(route('admin.adventure.edit', $deposit)); ?>" class="bg-blue-500 hover:bg-blue-600 text-white font-bold p-2 rounded-lg shadow-sm shadow-blue-500/10 transition-all text-xs flex items-center justify-center" title="Edit Plan">
                                        <i class="fas fa-edit"></i>
                                    </a>

                                    
                                    <form action="<?php echo e(route('admin.Adventures.destroy', $deposit->id)); ?>" method="POST" onsubmit="return confirm('Are you sure you want to delete this adventure?');" class="inline">
                                        <?php echo csrf_field(); ?>
                                    ...
                                    <a href="<?php echo e(route('admin.Adventures.investors', $deposit->id)); ?>" class="bg-slate-700 hover:bg-slate-800 text-white font-bold py-1.5 px-3 rounded-lg shadow-sm transition-all text-xs inline-flex items-center gap-1.5" title="View Investors">
                    ...
                    <?php if($Adventures->hasPages()): ?>
                        <tr class="bg-slate-50">
                            <td colspan="8" class="px-6 py-4">
                                <div class="flex justify-center">
                                    <?php echo e($Adventures->links()); ?>
...
<?php echo $__env->make('admin.sidebar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\bifonepo\mcu.focoin.eu\afonete\resources\views/admin/uvp/Adventures.blade.php ENDPATH**/ ?>