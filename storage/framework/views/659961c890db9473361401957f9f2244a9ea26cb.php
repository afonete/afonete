...
<div class="wrap">
    <div class="top">
        ...
        <div class="row">
            <a class="btn btn-dark" href="<?php echo e($deposit->package_type === 'FC' ? route('user.package') : route('user.venture')); ?>">Back to packages</a>
            <?php if($deposit->status === 'pending'): ?>
                <form method="POST" action="<?php echo e(route('payment.directPackage.cancel', $deposit->id)); ?>" style="display:inline;">
                    <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                <a class="btn btn-dark" href="<?php echo e(route('user.manual-deposit')); ?>">Use another payment option</a>
            <?php endif; ?>
            <a class="btn btn-dark" href="<?php echo e(route('user.dashboard')); ?>">Dashboard</a>
        </div>
    </div>

    <?php if(session('error')): ?>
        <div class="warn" style="margin-bottom:14px;"><?php echo e(session('error')); ?></div>
    <?php endif; ?>
    ...
    <div class="grid">
        <div class="card">
            <div class="card-h">
                ...
                <span class="muted"><?php echo e($networkLabel); ?> USDT address</span>
            </div>
            <div class="card-b" style="text-align:center;">
                <img class="qr" src="<?php echo e($qrUrl); ?>" alt="USDT <?php echo e($network); ?> payment QR code">
                <div class="info" style="text-align:left;">
                    <strong>Before sending, confirm:</strong><br>
                    Amount: <strong><?php echo e(number_format($amount, 2)); ?> USDT</strong><br>
                    Network: <strong>TRON / TRC20 only</strong><br>
                    Package: <strong><?php echo e($packageName); ?></strong>
                </div>
            </div>
        </div>
        ...
        <div class="card">
            <div class="card-h">
                <span id="statusBadge" class="pill <?php echo e($deposit->status === 'approved' ? 'approved' : ($isCancelled ? 'cancelled' : ($isExpired ? 'expired' : 'pending'))); ?>">
                    <?php echo e($isCancelled ? 'CANCELLED' : ($isExpired && $deposit->status === 'pending' ? 'EXPIRED' : strtoupper($deposit->status))); ?>

                </span>
                <h2 style="margin:12px 0 0;"><?php echo e($packageName); ?></h2>
                <div class="muted">Reference: <?php echo e($deposit->transaction_id); ?></div>
            </div>
            <div class="card-b">
                <div class="label">Exact amount to pay</div>
                <div class="amount"><?php echo e(number_format($amount, 2)); ?> USDT</div>
                ...
                <div class="warn">
                    ...
                    Example: if this invoice is <strong><?php echo e(number_format($amount, 2)); ?> USDT</strong> and your exchange charges a fee,
                    your wallet balance must cover <strong><?php echo e(number_format($amount, 2)); ?> USDT + the fee</strong>, so we still receive the full invoice amount.
                </div>

                <div class="timer">
                    <div class="label">Payment window</div>
                    <div id="countdown" class="timer-num"><?php echo e($secondsRemaining !== null ? gmdate('i:s', $secondsRemaining) : '15:00'); ?></div>
                ...
                <div class="label">USDT <?php echo e($network); ?> payment address</div>
                <div id="payAddress" class="value"><?php echo e($address); ?></div>
                <div class="row">
                    <button class="btn btn-green" type="button" data-copy="payAddress">Copy address</button>
                    <button class="btn btn-dark" type="button" data-copy-text="<?php echo e($paymentUrl); ?>">Copy payment URL</button>
                    <?php if($deposit->status === 'pending'): ?>
                        <form method="POST" action="<?php echo e(route('payment.directPackage.cancel', $deposit->id)); ?>" style="display:inline;">
                            <?php echo csrf_field(); ?>
                            <button class="btn btn-dark" type="submit" onclick="return confirm('Cancel this automatic payment order and switch to manual payment option?');">
                        ...
                        <form method="POST" action="<?php echo e(route('payment.directPackage.cancel', $deposit->id)); ?>" onsubmit="return confirm('Cancel this automatic TRC-20 payment invoice?');" style="display:inline;">
                            <?php echo csrf_field(); ?>
                ...
                <div class="warn">
                    <strong>Important:</strong> Only send <strong>USDT on <?php echo e($networkLabel); ?></strong>. Do not send through another network or any other asset to this address.
                </div>
                ...
                <?php if($isCancelled): ?>
                    <div class="warn">
                        ...
                        <div class="row" style="margin-top:10px;">
                            <a class="btn btn-dark" href="<?php echo e(route('user.manual-deposit')); ?>">Go to manual deposit</a>
                        </div>
                    </div>
                <?php endif; ?>

                <div id="expiredBox" class="warn" style="<?php echo e($isExpired ? '' : 'display:none;'); ?>">
                    ...
                    <form action="<?php echo e(route('payment.directPackage')); ?>" method="POST" style="margin-top:10px;">
                        <?php echo csrf_field(); ?>
                        <input type="hidden" name="package_type" value="<?php echo e($deposit->package_type); ?>">
                        <input type="hidden" name="package_id" value="<?php echo e($deposit->package_id); ?>">
                        <input type="hidden" name="amount" value="<?php echo e(number_format($amount, 2, '.', '')); ?>">
                        <input type="hidden" name="network" value="<?php echo e($network); ?>">
                ...
                <div class="info">
                    ...
                    <ol>
                        <li>Pay <strong><?php echo e(number_format($amount, 2)); ?> USDT</strong> to the address above before the 15-minute timer ends.</li>
                        <li>The system scans the TRON blockchain automatically every 5 minutes.</li>
                        <li>If the payment is received inside the timer window, your <strong><?php echo e($packageName); ?></strong> package is activated automatically.</li>
                    </ol>
                </div>
...
(function(){
    ...
    var statusUrl = <?php echo json_encode($statusUrl, 15, 512) ?>;
    var expiresAt = <?php echo json_encode($expiresAtIso, 15, 512) ?>;
...
<?php /**PATH C:\xampp\htdocs\bifonepo\mcu.focoin.eu\afonete\resources\views/user/direct-tron-package-payment.blade.php ENDPATH**/ ?>