            ...
            <div class="bg-slate-900 text-white rounded-2xl p-4 sm:p-6 shadow-md border border-slate-800 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                ...
                <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full sm:w-auto">
                    <div class="bg-slate-800 px-4 py-2.5 rounded-xl border border-slate-700 text-left sm:text-right w-full sm:w-auto">
                        ...
                        <span class="text-lg sm:text-xl font-extrabold text-white">$<?php echo e(number_format($depositBalance ?? 0, 2)); ?></span>
                    </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="flex items-center">
                        <input type="text" id="leftLink" value="<?php echo e(url('/register?referral=' . (auth()->user()->activation ?? auth()->user()->user ?? auth()->id()) . '&side=LEFT')); ?>" class="w-full min-w-0 px-3 py-2 text-xs border border-slate-300 rounded-l-xl font-mono text-slate-800" readonly>
                ...
                <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                    ...
                    <div class="flex items-center">
                        <input type="text" id="rightLink" value="<?php echo e(url('/register?referral=' . (auth()->user()->activation ?? auth()->user()->user ?? auth()->id()) . '&side=RIGHT')); ?>" class="w-full min-w-0 px-3 py-2 text-xs border border-slate-300 rounded-l-xl font-mono text-slate-800" readonly>
                ...
                <div id="visualTreeView" class="space-y-6">
                    ...
                    <div class="flex justify-center">
                        <div class="bg-slate-900 text-white p-4 rounded-2xl border-2 border-amber-400 shadow-lg text-center max-w-xs w-full">
                            ...
                            <h5 class="text-base font-extrabold text-white mt-1">@ <?php echo e(Auth::user()->user); ?></h5>
                            <div class="text-xs text-slate-300 font-mono mt-0.5">Code: <?php echo e(Auth::user()->transfer_code); ?></div>
                            <div class="mt-2">
                                <span class="bg-slate-800 text-amber-300 text-xs font-extrabold px-2.5 py-1 rounded-lg border border-slate-700">
                                    <?php echo e(strtoupper(Auth::user()->has_paid_package ?: 'FREE / STANDARD')); ?>
                        ...
                        <div class="bg-sky-50/60 rounded-2xl p-5 border border-sky-200">
                            <div class="flex items-center justify-between border-b border-sky-200 pb-3 mb-3">
                                ...
                                <span class="bg-sky-600 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                                    <?php echo e(count($leftMembers)); ?> <?php echo e(Str::plural('Member', count($leftMembers))); ?>

                                </span>
                            </div>

                            <?php if(count($leftMembers) > 3): ?>
                                <div class="mb-3">
                            ...
                            <div class="space-y-3 max-h-[500px] overflow-y-auto pr-1" id="leftBranchContainer">
                                <?php $__empty_1 = true; $__currentLoopData = $leftMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <div class="left-member-item bg-white p-3.5 rounded-xl border border-sky-200 shadow-sm flex items-center justify-between gap-3 <?php echo e($index >= 5 ? 'hidden left-extra-item' : ''); ?>" data-search="<?php echo e(strtolower($m['user'] . ' ' . $m['transfer_code'] . ' ' . $m['name'])); ?>">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <div class="text-xs font-extrabold text-slate-900 flex items-center gap-1.5 flex-wrap">
                                                    ...
                                                    <?php else: ?>
                                                        <span class="bg-slate-100 text-slate-700 text-[9px] font-extrabold px-1.5 py-0.5 rounded border border-slate-200">L<?php echo e($m['level']); ?> Indirect</span>
                                                    <?php endif; ?>
                                                </div>
                                                <div class="text-[11px] text-slate-500 font-mono mt-0.5">Code: <?php echo e($m['transfer_code']); ?></div>
                                                <?php if(!$m['is_direct'] && !empty($m['sponsor_user'])): ?>
                                                    <div class="text-[10px] text-indigo-600 font-medium">by @ <?php echo e($m['sponsor_user']); ?></div>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                        <div class="text-right shrink-0">
                                            <span class="bg-sky-100 text-sky-800 text-[10px] font-extrabold px-2 py-0.5 rounded border border-sky-200 uppercase block mb-1">
                                                <?php echo e($m['package']); ?>

                                            </span>
                                            <small class="text-slate-400 text-[10px]"><?php echo e($m['joined']); ?></small>
                                        </div>
                                    </div>
                            ...
                            <?php if(count($leftMembers) > 5): ?>
                                <button type="button" onclick="toggleBranchLimit('left')" id="btnLeftToggle" class="w-full mt-2.5 py-2 bg-sky-100 hover:bg-sky-200 text-sky-800 text-xs font-bold rounded-xl border border-sky-200 transition flex items-center justify-center gap-1.5">
                                    ...
                                    <span id="textLeftToggle">Show More (+<?php echo e(count($leftMembers) - 5); ?> Remaining)</span>
                                </button>
                            <?php endif; ?>
                        ...
                        <div class="bg-indigo-50/60 rounded-2xl p-5 border border-indigo-200">
                            <div class="flex items-center justify-between border-b border-indigo-200 pb-3 mb-3">
                                ...
                                <span class="bg-indigo-600 text-white text-xs font-bold px-2.5 py-1 rounded-full">
                                    <?php echo e(count($rightMembers)); ?> <?php echo e(Str::plural('Member', count($rightMembers))); ?>

                                </span>
                            </div>

                            <?php if(count($rightMembers) > 3): ?>
                                <div class="mb-3">
                            ...
                            <div class="space-y-3 max-h-[500px] overflow-y-auto pr-1" id="rightBranchContainer">
                                <?php $__empty_1 = true; $__currentLoopData = $rightMembers; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <div class="right-member-item bg-white p-3.5 rounded-xl border border-indigo-200 shadow-sm flex items-center justify-between gap-3 <?php echo e($index >= 5 ? 'hidden right-extra-item' : ''); ?>" data-search="<?php echo e(strtolower($m['user'] . ' ' . $m['transfer_code'] . ' ' . $m['name'])); ?>">
                                        <div class="flex items-center gap-3">
                            ...
                            <?php if(count($rightMembers) > 5): ?>
                                <button type="button" onclick="toggleBranchLimit('right')" id="btnRightToggle" class="w-full mt-2.5 py-2 bg-indigo-100 hover:bg-indigo-200 text-indigo-800 text-xs font-bold rounded-xl border border-indigo-200 transition flex items-center justify-center gap-1.5">
                                    ...
                                    <span id="textRightToggle">Show More (+<?php echo e(count($rightMembers) - 5); ?> Remaining)</span>
                                </button>
                            <?php endif; ?>
...
function initGoJsDiagram() {
    ...
    const leftMembers  = <?php echo json_encode($leftMembers, 15, 512) ?>;
    const rightMembers = <?php echo json_encode($rightMembers, 15, 512) ?>;
    const rootUserId   = <?php echo json_encode(Auth::id(), 15, 512) ?>;

    const nodes = [
        { key: rootUserId, name: "@" + <?php echo json_encode(Auth::user()->user, 15, 512) ?> + " (You)", title: "Root Sponsor", package: <?php echo json_encode(strtoupper(Auth::user()->has_paid_package ?: 'FREE / STANDARD'), 15, 512) ?>, fill: "#0f172a", stroke: "#f59e0b", userId: rootUserId },
