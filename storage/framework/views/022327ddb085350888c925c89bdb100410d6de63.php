...
<div class="p-4 sm:p-6 bg-slate-50 min-h-screen">
    ...
    <div class="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
            ...
            <nav class="text-sm text-slate-500 mt-1">
                <ol class="inline-flex items-center space-x-2">
                    <li><a href="<?php echo e(route('admin.dashboard')); ?>" class="hover:text-slate-700"><i class="fa fa-home mr-1"></i>Dashboard</a></li>
        ...
        <div class="flex items-center gap-2 text-xs">
            ...
            <?php if(!empty($methodFilter)): ?>
                <span class="px-2.5 py-1 rounded-full bg-slate-800 text-white font-semibold uppercase text-[10px]"><?php echo e($methodFilter); ?></span>
            <?php endif; ?>
        </div>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm px-4 py-4 border-l-[5px] border-amber-400">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-800 mt-1"><?php echo e($pending->count()); ?></p>
                    <p class="text-xs text-amber-600 font-mono mt-1">$<?php echo e(number_format($pendingTotal,2)); ?></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm px-4 py-4 border-l-[5px] border-sky-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-800 mt-1"><?php echo e($processing->count()); ?></p>
                    <p class="text-xs text-slate-500 mt-1">$<?php echo e(number_format($processingTotal,2)); ?> queued</p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm px-4 py-4 border-l-[5px] border-emerald-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-800 mt-1"><?php echo e($completed->total()); ?></p>
                    <p class="text-xs text-slate-500 mt-1">page <?php echo e($completed->currentPage()); ?> / <?php echo e($completed->lastPage()); ?></p>
                </div>
        ...
        <div class="bg-white rounded-lg shadow-sm px-4 py-4 border-l-[5px] border-indigo-500">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-2xl font-bold text-slate-800 mt-1 font-mono">$<?php echo e(number_format($completedPageTotal,2)); ?></p>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-3 mb-5 flex flex-wrap items-center gap-2">
        ...
        <a href="<?php echo e(route('admin.withdrawal')); ?>"
           class="px-3 py-1.5 text-xs font-semibold rounded-full <?php echo e(empty($methodFilter) ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-700 hover:bg-slate-200'); ?>">
            All
        </a>
        <a href="<?php echo e(route('admin.withdrawal', ['method' => 'crypto'])); ?>"
           class="px-3 py-1.5 text-xs font-semibold rounded-full inline-flex items-center gap-1.5 <?php echo e(($methodFilter ?? '')==='crypto' ? 'bg-indigo-600 text-white' : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'); ?>">
            <i class="fab fa-bitcoin"></i> Crypto
        </a>
        <a href="<?php echo e(route('admin.withdrawal', ['method' => 'advcash'])); ?>"
           class="px-3 py-1.5 text-xs font-semibold rounded-full inline-flex items-center gap-1.5 <?php echo e(($methodFilter ?? '')==='advcash' ? 'bg-amber-500 text-white' : 'bg-amber-50 text-amber-700 hover:bg-amber-100'); ?>">
            <i class="fas fa-money-bill-wave"></i> Advcash
        </a>
        <a href="<?php echo e(route('admin.withdrawal', ['method' => 'perfect_money'])); ?>"
           class="px-3 py-1.5 text-xs font-semibold rounded-full inline-flex items-center gap-1.5 <?php echo e(($methodFilter ?? '')==='perfect_money' ? 'bg-sky-600 text-white' : 'bg-sky-50 text-sky-700 hover:bg-sky-100'); ?>">
            <i class="fas fa-coins"></i> Perfect Money
        </a>
        <?php if(!empty($methodFilter)): ?>
            <a href="<?php echo e(route('admin.withdrawal')); ?>" class="text-xs text-slate-500 hover:text-slate-700 ml-2">Clear ×</a>
        <?php endif; ?>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="px-4 py-3 border-b border-slate-200 bg-amber-50 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs px-2 py-0.5 rounded-full bg-amber-200 text-amber-900 font-bold"><?php echo e($pending->count()); ?></span>
            </div>
        ...
        <?php if($pending->isEmpty()): ?>
            <div class="p-8 text-center text-slate-400">
                ...
                <p class="text-sm">No pending withdrawals<?php echo e($methodFilter ? ' for '.$methodFilter : ''); ?>.</p>
            </div>
        <?php else: ?>
        <div class="overflow-x-auto">
            <table class="w-full text-sm" id="pendingTable" style="min-width: 960px;">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    ?>
                    <tr class="hover:bg-slate-50 withdrawal-row" data-search="<?php echo e(strtolower(($w->user->name ?? '').' '.($w->user->email ?? '').' '.$w->wallet_address.' '.$w->transaction_no)); ?>">
                        ...
                        <td class="px-4 py-3 align-top">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="min-w-0">
                                    <div class="font-semibold text-slate-800 truncate"><?php echo e($w->user->name ?? '—'); ?></div>
                                    <div class="text-[11px] text-slate-500 truncate"><?php echo e($w->user->email ?? ''); ?></div>
                                    <div class="font-mono text-[10px] text-slate-400 mt-1 truncate max-w-[200px]" title="<?php echo e($w->transaction_no); ?>">
                        ...
                        <td class="px-4 py-3 align-top">
                            <div class="font-mono font-bold text-rose-600 text-[15px]">$<?php echo e(number_format($w->amount,2)); ?></div>
                            <div class="mt-1">
                                <span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-semibold ring-1 ring-inset <?php echo e($methodColor); ?>">
                                    <?php echo e(method_exists($w, 'methodLabel') ? $w->methodLabel() : ucfirst($w->method)); ?>

                                </span>
                            </div>
                            <div class="text-[11px] text-slate-500 mt-1"><?php echo e(strtoupper($w->currency)); ?><?php echo e($w->network ? ' · '.$w->network : ''); ?></div>
                        </td>
                        ...
                        <td class="px-4 py-3 align-top text-[12px] text-slate-600 whitespace-nowrap">
                            <?php echo e($w->created_at->format('M d, H:i')); ?><br>
                            <span class="text-[11px] text-slate-400"><?php echo e($w->created_at->diffForHumans()); ?></span>
                        </td>
                        
                        <td class="px-4 py-3 align-top text-right">
                            <form method="POST" action="<?php echo e(route('admin.withdrawal.approve')); ?>" class="inline-block text-left">
                                <?php echo csrf_field(); ?>
                                ...
                                <div class="flex flex-col gap-1.5 w-[340px] ml-auto">
                                    ...
                                    <div class="flex justify-end gap-1.5">
                                        <button type="button"
                                                onclick="openReject(<?php echo e($w->id); ?>, '<?php echo e(addslashes($w->user->name ?? 'User')); ?>', <?php echo e($w->amount); ?>)"
                                                class="px-3 py-1.5 text-xs font-semibold rounded-lg border border-rose-300 text-rose-700 hover:bg-rose-50">
                                            <i class="fa fa-times mr-1"></i> Reject
                                        </button>
                                        <button type="submit"
                                                onclick="return confirm('Approve withdrawal of $<?php echo e(number_format($w->amount,2)); ?> for <?php echo e(addslashes($w->user->name ?? '')); ?>?')"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="px-4 py-3 border-b border-slate-200 bg-sky-50 flex items-center justify-between">
            <div class="flex items-center gap-2">
                ...
                <span class="text-xs px-2 py-0.5 rounded-full bg-sky-200 text-sky-900 font-bold"><?php echo e($processing->count()); ?></span>
            </div>
            <span class="text-[11px] text-sky-700">Automatic blockchain payout in progress</span>
        </div>

        <?php if($processing->isEmpty()): ?>
            ...
            <table class="w-full text-sm" style="min-width: 900px;">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3">
                            <div class="font-semibold text-slate-800"><?php echo e($w->user->name ?? '—'); ?></div>
                            <div class="text-[11px] text-slate-500"><?php echo e($w->user->email ?? ''); ?></div>
                            <div class="font-mono text-[10px] text-slate-400"><?php echo e($w->transaction_no); ?></div>
                        </td>
                        <td class="px-4 py-3 font-mono font-semibold text-slate-800">$<?php echo e(number_format($w->amount,2)); ?></td>
                        <td class="px-4 py-3 max-w-[280px]">
                            <div class="text-[11px] text-slate-600"><?php echo e(strtoupper($w->currency)); ?> <?php echo e($w->network); ?></div>
                            <div class="font-mono text-[11px] text-slate-700 break-all"><?php echo e($w->wallet_address); ?></div>
                        </td>
                        <td class="px-4 py-3">
                            <span class="text-sm font-semibold text-slate-800"><?php echo e($w->attempts ?? 0); ?></span>
                            <span class="text-[11px] text-slate-500"> attempts</span><br>
                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold <?php echo e(($w->risk_score ?? 0) >= 50 ? 'bg-rose-100 text-rose-700' : 'bg-slate-100 text-slate-600'); ?>">
                                risk <?php echo e($w->risk_score ?? 0); ?>

                            </span>
                            <?php if(!empty($w->risk_flags)): ?>
                                <div class="text-[10px] text-slate-400 mt-1"><?php echo e(is_array($w->risk_flags) ? implode(', ', $w->risk_flags) : $w->risk_flags); ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-3 text-[12px] text-slate-600">
                            <?php echo e($w->last_attempt_at ? \Carbon\Carbon::parse($w->last_attempt_at)->format('M d H:i') : '—'); ?>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm" style="min-width: 920px;">
                ...
                <tbody class="divide-y divide-slate-100">
                    ...
                    <tr class="hover:bg-slate-50">
                        <td class="px-4 py-3">
                            <div class="font-semibold text-slate-800 text-[13px]"><?php echo e($w->user->name ?? '—'); ?></div>
                            <div class="text-[11px] text-slate-500"><?php echo e($w->user->email ?? ''); ?></div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="font-mono font-semibold text-slate-800">$<?php echo e(number_format($w->amount,2)); ?></div>
                            <div class="text-[11px] text-slate-500"><?php echo e(method_exists($w,'methodLabel') ? $w->methodLabel() : $w->method); ?> · <?php echo e($w->currency); ?><?php echo e($w->network ? ' · '.$w->network : ''); ?></div>
                        </td>
                        <td class="px-4 py-3 max-w-[240px]">
                            <div class="font-mono text-[11px] text-slate-600 break-all"><?php echo e(\Illuminate\Support\Str::limit($w->wallet_address, 42)); ?></div>
                            <div class="font-mono text-[10px] text-slate-400 mt-0.5"><?php echo e($w->transaction_no); ?></div>
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2 py-0.5 rounded text-[10px] font-semibold bg-slate-100 text-slate-700"><?php echo e(method_exists($w,'typeLabel') ? $w->typeLabel() : ($w->type ?? '—')); ?></span>
                            <?php if($w->admin_note): ?>
                                <div class="text-[11px] text-slate-500 mt-1 max-w-[220px] truncate" title="<?php echo e($w->admin_note); ?>"><?php echo e($w->admin_note); ?></div>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-3 text-[12px] text-slate-600 whitespace-nowrap">
                            <?php echo e($w->created_at->format('M d, Y')); ?><br>
                            <span class="text-[11px] text-slate-400"><?php echo e($w->created_at->format('H:i')); ?></span>
                        </td>
                    </tr>
                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                </tbody>
        ...
        <div class="px-4 py-3 border-t border-slate-200 bg-slate-50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xs text-slate-600">
            <div>
                Showing <b class="text-slate-800"><?php echo e($completed->firstItem()); ?></b>–<b class="text-slate-800"><?php echo e($completed->lastItem()); ?></b>
                of <b class="text-slate-800"><?php echo e($completed->total()); ?></b>
            </div>
            <div><?php echo e($completed->links()); ?></div>
        </div>
        <?php endif; ?>
    </div>
