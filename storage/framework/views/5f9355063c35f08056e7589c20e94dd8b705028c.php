...
<div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8 max-w-7xl">
    ...
    <header class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 py-6 px-6 rounded-2xl mb-8 shadow-sm">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <a href="<?php echo e(route('admin.announcements.create')); ?>" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition duration-150 text-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden shadow-sm">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php $__currentLoopData = $announcements; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $ann): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-bold"><?php echo e($index + 1); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-800 font-extrabold"><?php echo e($ann->title); ?></td>
                            <td class="px-6 py-4 text-sm text-gray-600 max-w-sm truncate"><?php echo e($ann->content); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?php echo e($ann->sort_order); ?></td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center font-medium">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="<?php echo e(route('admin.announcements.toggle', $ann->id)); ?>" class="inline-flex items-center justify-center bg-amber-500 hover:bg-amber-600 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition shadow-sm">
                                        <i class="fas fa-toggle-on mr-1.5"></i> Toggle
                                    </a>
                                    <a href="<?php echo e(route('admin.announcements.delete', $ann->id)); ?>" onclick="return confirm('Delete this announcement?')" class="inline-flex items-center justify-center bg-red-600 hover:bg-red-700 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition shadow-sm">
...
<?php echo $__env->make('admin.sidebar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\bifonepo\mcu.focoin.eu\afonete\resources\views/admin/announcements/index.blade.php ENDPATH**/ ?>