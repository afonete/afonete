...
<div class="container-fluid py-4 px-4 bg-slate-50 min-h-screen">
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
            ...
            <form method="POST" action="<?php echo e(route('admin.zoom.store')); ?>" class="space-y-4">
                <?php echo csrf_field(); ?>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="md:col-span-2">
                        ...
                        <input type="text" name="topic" required placeholder="e.g. Official Bifonex Weekly Live Coaching &amp; Presentation" class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs font-bold text-slate-800 focus:ring-indigo-500 focus:border-indigo-500" value="<?php echo e(old('topic', 'Official Bifonex Live Zoom Presentation')); ?>">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="url" name="zoom_link" required placeholder="https://us02web.zoom.us/j/123456789..." class="w-full px-3 py-2.5 rounded-xl border border-slate-300 text-xs font-mono font-bold text-slate-900 focus:ring-indigo-500 focus:border-indigo-500" value="<?php echo e(old('zoom_link')); ?>">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-700 mb-1">Meeting ID (Optional)</label>
                        <input type="text" name="meeting_id" placeholder="e.g. 849 1234 5678" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-mono text-slate-800" value="<?php echo e(old('meeting_id')); ?>">
                    </div>

                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-700 mb-1">Passcode (Optional)</label>
                        <input type="text" name="passcode" placeholder="e.g. 123456" class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs font-mono text-slate-800" value="<?php echo e(old('passcode')); ?>">
                    </div>

                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold uppercase text-slate-700 mb-1">Description / Notes (Optional)</label>
                        <textarea name="description" rows="2" placeholder="e.g. Join our executive leadership team live for system updates and Q&amp;A." class="w-full px-3 py-2 rounded-xl border border-slate-300 text-xs text-slate-800"><?php echo e(old('description')); ?></textarea>
                    </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 overflow-x-auto">
            <h3 class="text-base font-extrabold text-slate-900 border-b pb-3 mb-4 flex items-center justify-between">
                ...
                <span class="text-xs bg-slate-100 text-slate-700 font-bold px-3 py-1 rounded-full">Total: <?php echo e(count($meetings)); ?></span>
            </h3>
                        ...
                        <tr class="hover:bg-slate-50/80 transition">
                            <td class="py-3.5 px-4 font-bold text-slate-900">
                                ...
                                <?php if(!empty($m->description)): ?>
                                    <div class="text-[11px] text-slate-400 font-normal mt-0.5"><?php echo e($m->description); ?></div>
                                <?php endif; ?>
                            </td>
                            <td class="py-3.5 px-4 font-mono text-indigo-600 max-w-xs truncate">
                                <a href="<?php echo e($m->zoom_link); ?>" target="_blank" class="hover:underline flex items-center gap-1">
                                    <i class="fas fa-external-link-alt text-[10px]"></i> <?php echo e($m->zoom_link); ?>

                                </a>
                            </td>
                            <td class="py-3.5 px-4 font-mono text-slate-700">
                                <?php echo e($m->meeting_id ?: 'N/A'); ?> / <?php echo e($m->passcode ?: 'N/A'); ?>
                            ...
                            <td class="py-3.5 px-4 text-slate-400"><?php echo e($m->created_at ? $m->created_at->format('M d, Y H:i') : ''); ?></td>
                            <td class="py-3.5 px-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <?php if($m->status === 'active'): ?>
                                        <form method="POST" action="<?php echo e(route('admin.zoom.toggle', $m->id)); ?>" class="inline">
                                            <?php echo csrf_field(); ?>
                                    ...
                                    <form method="POST" action="<?php echo e(route('admin.zoom.delete', $m->id)); ?>" class="inline" onsubmit="return confirm('Delete this Zoom meeting record?')">
                                        <?php echo csrf_field(); ?>
...
<?php echo $__env->make('admin.sidebar', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\bifonepo\mcu.focoin.eu\afonete\resources\views/admin/zoom-meetings.blade.php ENDPATH**/ ?>