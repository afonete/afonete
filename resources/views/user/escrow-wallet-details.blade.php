        ...
        <div class="container-fluid py-4 px-4 max-w-7xl mx-auto">
            ...
            <div class="p-4 mb-4 shadow-lg d-flex align-items-center justify-content-between flex-wrap gap-3 vault-card">
                ...
                <div class="d-flex align-items-center gap-2">
                    <a href="{{ route('user.dashboard.activate') }}" class="btn btn-warning text-dark font-weight-bold px-3 py-2" style="border-radius: 8px;">
                    ...
                    <div class="card p-3 shadow-sm h-100 border-0" style="background: #1e293b; border-left: 4px solid #10b981 !important; border-radius: 14px;">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h3 class="font-weight-bold text-success mb-0 mt-1">{{ number_format($availableBal, 0) }}</h3>
                                <small class="text-muted">{{ $tokenSymbol }}</small>
                            </div>
                    ...
                    <div class="card p-3 shadow-sm h-100 border-0" style="background: #1e293b; border-left: 4px solid #3b82f6 !important; border-radius: 14px;">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <h3 class="font-weight-bold text-info mb-0 mt-1">{{ number_format($releasedTokens, 0) }}</h3>
                                <small class="text-muted">{{ $releasedCount }} / 12 Monthly Releases Done</small>
                            </div>
            ...
            <div class="tab-content" id="escrowTabsContent">
                ...
                {{-- TAB 1: 12 MONTHLY PACKAGE RELEASES SCHEDULE --}}
                    ...
                    <div class="card vault-card p-4">
                        ...
                        @if(empty($installments) || count($installments) === 0)
                            <div class="text-center p-5 text-muted">
                                ...
                                <a href="{{ route('investment-package') }}" class="btn btn-warning text-dark font-weight-bold px-4">
                ...
                {{-- TAB 2: MULTI-YEAR ESCROW STAKING VAULT (1-5 YEARS) --}}
                <div class="tab-pane fade" id="tab-staking" role="tabpanel">
                    <div class="card vault-card p-4">
                        ...
                        <form method="POST" action="{{ route('user.token.available-to-escrow-staking') }}" class="js-transaction-password-form">
                            @csrf
                            ...
                            <div class="mb-4 max-w-xl">
                                ...
                                <div class="input-group input-group-lg">
                                    <input type="number" name="amount" id="tabStakeAmount" class="form-control bg-dark text-white border-secondary font-weight-bold"
                                           min="1" step="1" max="{{ $availableBal }}" required placeholder="e.g. 10000" style="font-size: 1.25rem;">
                                    <span class="input-group-text bg-secondary border-secondary text-white font-weight-bold">{{ $tokenSymbol }}</span>
                                </div>
                                <div class="d-flex justify-content-between mt-1 text-muted small">
                                    <span>Available Tokens: <strong class="text-success">{{ number_format($availableBal, 0) }} {{ $tokenSymbol }}</strong></span>
                                    <button type="button" class="btn btn-link text-warning p-0 text-decoration-none small" onclick="document.getElementById('tabStakeAmount').value={{ (int)$availableBal }}; updateTabPreview();">Lock Max Balance</button>
                                </div>
                            </div>

                            {{-- Step 2: Select Duration Cards --}}
                            <div class="mb-4">
                                ...
                                <div class="row g-3">
                                    <div class="col-6 col-md">
                                        <div class="year-card selected" onclick="selectYearCard(1, {{ $y1 }}, this)">
                                            <span class="year-yield-badge mb-2 d-inline-block">+{{ $y1 }}% Profit</span>
                                            <h4 class="font-weight-bold text-white mb-1">1 Year</h4>
                                            <span class="text-muted small d-block">Short-term Vault</span>
                                        </div>
                                    </div>
                                    <div class="col-6 col-md">
                                        <div class="year-card" onclick="selectYearCard(2, {{ $y2 }}, this)">
                                            <span class="year-yield-badge mb-2 d-inline-block">+{{ $y2 }}% Profit</span>
                                            <h4 class="font-weight-bold text-white mb-1">2 Years</h4>
                                            <span class="text-muted small d-block">Medium Vault</span>
                                        </div>
                                    </div>
                                    <div class="col-6 col-md">
                                        <div class="year-card" onclick="selectYearCard(3, {{ $y3 }}, this)">
                                            <span class="year-yield-badge mb-2 d-inline-block">+{{ $y3 }}% Profit</span>
                                            <h4 class="font-weight-bold text-white mb-1">3 Years</h4>
                                            <span class="text-muted small d-block">Growth Vault</span>
                                        </div>
                                    </div>
                                    <div class="col-6 col-md">
                                        <div class="year-card" onclick="selectYearCard(4, {{ $y4 }}, this)">
                                            <span class="year-yield-badge mb-2 d-inline-block">+{{ $y4 }}% Profit</span>
                                            <h4 class="font-weight-bold text-white mb-1">4 Years</h4>
                                            <span class="text-muted small d-block">High Yield Vault</span>
                                        </div>
                                    </div>
                                    <div class="col-6 col-md">
                                        <div class="year-card" onclick="selectYearCard(5, {{ $y5 }}, this)">
                                            <span class="year-yield-badge mb-2 d-inline-block">+{{ $y5 }}% Profit</span>
                            ...
                            {{-- Step 3: Calculation & Maturity Summary Box --}}
                            <div class="p-4 bg-dark rounded-3 border border-secondary mb-4 max-w-2xl">
                                ...
                                <div class="row text-center g-3">
                                    <div class="col-4 border-end border-secondary">
                                        ...
                                        <span class="text-muted d-block text-xs">{{ $tokenSymbol }}</span>
                                    </div>
                                    <div class="col-4 border-end border-secondary">
                            ...
                            <button type="submit" class="btn btn-warning btn-lg text-dark font-weight-bold px-5 py-3 shadow-lg" {{ $availableBal < 1 ? 'disabled' : '' }}>
                ...
                <div class="tab-pane fade" id="tab-history" role="tabpanel">
                    <div class="card vault-card p-4">
                        <div class="border-bottom border-secondary pb-3 mb-3 d-flex justify-content-between align-items-center flex-wrap gap-2">
                            ...
                            <span class="badge bg-success font-weight-bold px-3 py-2 fs-6">{{ count($stakings ?? []) }} Staking Vaults</span>
                        </div>

                        @if(empty($stakings) || count($stakings) === 0)
                            <div class="text-center p-5 text-muted">
                                    ...
                                    <tbody>
                                        @foreach($stakings as $stk)
                                            ...
                                            <tr class="border-bottom border-secondary">
                                                ...
                                                <td class="align-middle font-weight-bold text-white">{{ number_format((float)$stk->principal_amount) }} {{ $tokenSymbol }}</td>
                                                <td class="align-middle font-weight-bold text-success">+{{ (float)$stk->yield_percent }}%</td>
                                                <td class="align-middle font-weight-bold text-success">+{{ number_format((float)$stk->profit_amount) }} {{ $tokenSymbol }}</td>
                                                <td class="align-middle font-weight-bold text-warning fs-6">{{ number_format((float)$stk->total_staked) }} {{ $tokenSymbol }}</td>
                                                <td class="align-middle"><span class="badge bg-primary px-2.5 py-1.5">{{ $stk->lock_years }} Year(s)</span></td>
                                                <td class="align-middle text-light">{{ \Carbon\Carbon::parse($stk->release_date)->format('Y-m-d') }}</td>
                                                <td class="align-middle">
                                                    @if($isDone)
                                                        <span class="badge bg-success text-white px-3 py-1.5"><i class="fas fa-check-circle me-1"></i>Released to Available Tokens</span>
                                                    @else
