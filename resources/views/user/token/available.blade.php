...
<div class="container-fluid py-4">
    ...
    <div class="row mt-3">
        {{-- Card 1: Move to Free Token Wallet --}}
                ...
                <div class="card-body">
                    <div class="row text-center mb-3">
                        <div class="col-6 border-right">
                            ...
                            <div class="font-weight-bold text-success" style="font-size:1.3rem;">{{ number_format($availableBal, 0) }}</div>
        ...
        {{-- Card 2: Escrow Staking (1 to 5 Years Lock) --}}
        <div class="col-md-6 mb-4">
            <div class="card shadow-sm border-0 h-100">
                ...
                <div class="card-body">
                    ...
                    <form method="POST" action="{{ route('user.token.available-to-escrow-staking') }}">
                        @csrf
                        ...
                        <div class="form-group mb-3">
                            ...
                            <select name="years" id="stakeYears" class="form-control font-weight-bold" required>
                                <option value="1" data-yield="{{ $y1 }}">1 Year (+{{ $y1 }}% Profit)</option>
                                <option value="2" data-yield="{{ $y2 }}">2 Years (+{{ $y2 }}% Profit)</option>
                                <option value="3" data-yield="{{ $y3 }}">3 Years (+{{ $y3 }}% Profit)</option>
                                <option value="4" data-yield="{{ $y4 }}">4 Years (+{{ $y4 }}% Profit)</option>
                                <option value="5" data-yield="{{ $y5 }}">5 Years (+{{ $y5 }}% Profit)</option>
                            </select>
                        </div>
                        ...
                        <div class="p-2.5 bg-light rounded border mb-3 small">
                            <div class="d-flex justify-content-between mb-1">
                                ...
                                <span class="font-weight-bold" id="prevPrincipal">0 {{ $symbol }}</span>
                            </div>
                            <div class="d-flex justify-content-between mb-1">
                                <span class="text-muted">Bonus Profit:</span>
                                <span class="font-weight-bold text-success" id="prevProfit">+0 {{ $symbol }}</span>
                            </div>
                            <div class="d-flex justify-content-between border-top pt-1 font-weight-bold">
                                <span>Total Released to Available Tokens:</span>
                                <span class="text-primary" id="prevTotal">0 {{ $symbol }}</span>
                            </div>
                        </div>
