...
<body>
    ...
    <div x-data="walletApp()" class="min-h-screen bg-gray-900  text-white p-8">
        ...
        {{-- Manual Withdraw Notice/Link Banner --}}
        @if(isset($settings->auto_withdrawals_enabled) && $settings->auto_withdrawals_enabled)
            <div class="bg-indigo-950/40 border border-indigo-800/40 text-indigo-300 p-3.5 rounded-xl mb-6 flex justify-between items-center text-sm shadow-sm max-w-2xl mx-auto">
                ...
                <a href="{{ route('user.dashboard.manual_withdraw') }}" class="text-xs bg-indigo-800 hover:bg-indigo-700 text-white font-bold py-1.5 px-3 rounded-lg ml-3 transition-all flex-shrink-0" style="text-decoration:none;">
        ...
        <div class="text-center mb-8">
            ...
            <div class="text-yellow-400 text-4xl font-bold mt-2">{{$cashout}} USDT</div>
        ...
        <div x-cloak x-show="showSendModal" x-transition class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-lg max-w-md w-full">
                ...
                {{-- CONDITIONAL VIEW: Based on Admin settings enabling/disabling automatic withdraw --}}
                @if(isset($settings->auto_withdrawals_enabled) && $settings->auto_withdrawals_enabled)
                    
                    {{-- 1. AUTOMATIC TRC-20 WORKFLOW --}}
                    ...
                    <div class="space-y-3 text-left">
                        ...
                        <div class="mt-4 pt-3 border-t border-gray-700 text-center text-xs">
                            ...
                            <a href="{{ route('user.dashboard.manual_withdraw') }}" class="block text-yellow-500 hover:text-yellow-400 font-bold mt-1 underline">
                ...
                @else
                    ...
                    {{-- 2. MANUAL WORKFLOW (WITH SELECTABLE METHOD TABS) --}}
                    <div class="flex justify-between items-center mb-3">
                        <h3 class="text-xl font-bold text-gray-100 flex items-center gap-2">
        ...
        <div x-cloak x-show="showReceiveModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-gray-800 p-6 rounded-lg max-w-md w-full">
                <form action="{{route("user.receive")}}" method="GET">
                    @csrf
