        ...
        <section class="content">
            <div class="container-fluid py-4 px-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <a href="{{ route('investment-package') }}" class="btn btn-warning font-weight-bold">
                        <i class="fas fa-shopping-cart mr-1"></i>Buy Package Codes
                    </a>
                </div>

                {{-- Current Active Package Notice --}}
                <div class="card bg-dark text-white border-secondary mb-4 shadow-sm">
                    <div class="card-body d-flex align-items-center justify-content-between">
                        <div>
                            ...
                            <span class="fs-4 font-weight-bold text-success">{{ strtoupper($user->has_paid_package ?? 'Free') }}</span>
                        </div>
                        <div>
                            <a href="{{ route('user.dashboard.activate') }}" class="btn btn-outline-info btn-sm font-weight-bold">
                ...
                {{-- All Purchased Activation Codes Table --}}
                <div class="card shadow-sm border-0 mb-4" style="background:#1F2937; color:#fff; border-radius:10px;">
                    <div class="card-header bg-dark text-white font-weight-bold py-3 d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge badge-warning px-3 py-2 font-weight-bold text-dark">{{ count($myCodes ?? []) }} Codes</span>
                    </div>
                    <div class="card-body p-0">
                        @if(empty($myCodes) || count($myCodes) === 0)
                            <div class="text-center p-4 text-muted">
                                ...
                                <div class="mt-2">
                                    <a href="{{ route('investment-package') }}" class="btn btn-sm btn-warning font-weight-bold text-dark">
                        ...
                        @else
                            <div class="table-responsive">
                                <table class="table table-dark table-hover mb-0 text-center align-middle" style="background:transparent;">
                                    ...
                                    <tbody>
                                        @foreach($myCodes as $idx => $actCode)
                                            @php
                                            ...
                                            <tr class="border-bottom border-secondary">
                                                ...
                                                <td class="align-middle">
                                                    <code class="text-warning font-weight-bold bg-black px-2 py-1 rounded" id="hist-code-{{ $actCode->id }}">{{ $actCode->code }}</code>
                                                    <button type="button" class="btn btn-xs btn-outline-warning ml-2 font-weight-bold" onclick="copyHistCode('hist-code-{{ $actCode->id }}')">
                                                        <i class="fas fa-copy"></i> Copy
                                                    </button>
                                                </td>
                                                <td class="align-middle font-weight-bold text-white">{{ strtoupper($actCode->package) }}</td>
                                                <td class="align-middle text-success font-weight-bold">${{ number_format(\App\Models\FomLicenceMiner::cleanNum($actCode->price), 2) }}</td>
                                                ...
                                                <td class="align-middle">
                                                    @if(!$isUsed)
                                                        <form action="{{ route('user.investment-package.activate-code') }}" method="POST" class="d-inline">
                                                            @csrf
                                                            <input type="hidden" name="activation_id" value="{{ $actCode->id }}">
                                                            <input type="hidden" name="code" value="{{ $actCode->code }}">
