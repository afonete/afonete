...
<div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8 max-w-5xl">
    ...
    {{-- Back Button --}}
    <a href="{{ route('admin.team-leaders.index') }}" class="inline-flex items-center gap-2 text-sm text-blue-600 hover:text-blue-800 font-semibold mb-6 transition duration-150">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-500 text-sm mt-1 font-medium">
                        Team Leader ID #{{ $leader->id }} &middot; Applied {{ $leader->created_at->format('d M Y, H:i') }}
                    </p>
                </div>
            </div>
            <div>
                ...
                @elseif($leader->status === 'confirmed')
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800 break-all">{{ $leader->Email }}</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $leader->Phone }}</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $leader->Country }}</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-800">{{ $leader->created_at->format('d M Y, H:i') }}</p>
                        </div>
                    </div>
                    <div class="flex items-start gap-3">
                        <div class="w-8 h-8 {{ ($leader->leadership_level ?? 'TEAM_LEADER') === 'SUPER_LEADER' ? 'bg-yellow-50' : 'bg-blue-50' }} rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
                            <i class="fas fa-crown {{ ($leader->leadership_level ?? 'TEAM_LEADER') === 'SUPER_LEADER' ? 'text-yellow-600' : 'text-blue-600' }} text-sm"></i>
                        </div>
                        <div>
                            <p class="text-xs text-gray-400 font-bold uppercase tracking-wider">Leadership Level</p>
                            <p class="text-sm font-semibold {{ ($leader->leadership_level ?? 'TEAM_LEADER') === 'SUPER_LEADER' ? 'text-yellow-700' : 'text-blue-700' }}">
        ...
        <div class="lg:col-span-2 space-y-6">
                        ...
                        @if($leader->whatsapp)
                            <div class="flex items-stretch gap-2">
                                <div class="flex-1 bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-center">
                                    <a href="{{ $leader->whatsapp }}" target="_blank" rel="noopener"
                                ...
                                <button type="button"
                                        onclick="copyToClipboard('{{ $leader->whatsapp }}', this)"
                        ...
                        @if($leader->instagram)
                            <div class="flex items-stretch gap-2">
                                <div class="flex-1 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-center">
                                    <a href="{{ $leader->instagram }}" target="_blank" rel="noopener"
                                ...
                                <button type="button"
                                        onclick="copyToClipboard('{{ $leader->instagram }}', this)"
                                        class="inline-flex items-center justify-center px-4 bg-white hover:bg-gray-50 text-gray-700 rounded-xl border border-gray-300 transition duration-150 font-bold text-sm gap-1.5"
                                        title="Copy link">
                                    <i class="fas fa-copy"></i> Copy
                                </button>
                    ...
                    <div class="pt-3 border-t border-gray-100">
                        ...
                        @else
                            <div class="flex items-center gap-2 text-sm text-red-700 font-bold bg-red-50 border border-red-200 rounded-xl px-4 py-3">
                                ...
                                @if(!$leader->whatsapp) <span class="ml-1">WhatsApp</span>@endif
                                @if(!$leader->whatsapp && !$leader->instagram) <span>&amp;</span> @endif
                                @if(!$leader->instagram) <span>Telegram</span>@endif
            ...
            {{-- ═══════════════════════════════════════════════════════
                 PERFORMANCE MONITORING (confirmed leaders only)
                 ═══════════════════════════════════════════════════════ --}}
            @if($leader->status === 'confirmed' && isset($performance))
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 bg-gray-50">
                    <h3 class="font-bold text-slate-800 flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div class="bg-gray-50 rounded-xl p-3 text-center border">
                                ...
                                <p class="text-sm font-bold text-slate-800">{{ $performance['activation_date'] ? $performance['activation_date']->format('d M Y') : 'N/A' }}</p>
                            </div>
                            <div class="bg-gray-50 rounded-xl p-3 text-center border">
                                <p class="text-[10px] text-gray-400 font-bold uppercase">Expires</p>
                                <p class="text-sm font-bold text-slate-800">{{ $performance['expiry_date'] ? $performance['expiry_date']->format('d M Y') : 'N/A' }}</p>
                            </div>
                            <div class="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                                ...
                                <p class="text-sm font-extrabold text-blue-700">{{ $performance['days_elapsed'] }} / {{ $performance['duration_days'] }}</p>
                            </div>
                            <div class="rounded-xl p-3 text-center border {{ $performance['is_expired'] ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200' }}">
                                <p class="text-[10px] font-bold uppercase {{ $performance['is_expired'] ? 'text-red-400' : 'text-green-400' }}">Remaining</p>
                                <p class="text-sm font-extrabold {{ $performance['is_expired'] ? 'text-red-700' : 'text-green-700' }}">
                                    {{ $performance['is_expired'] ? 'EXPIRED' : $performance['days_remaining'] . ' days' }}
                                </p>
                            </div>
                        </div>
                        {{-- Progress bar --}}
                        @if($performance['duration_days'] > 0)
                        <div class="mt-2 w-full bg-gray-200 rounded-full h-2">
                            <div class="h-2 rounded-full transition-all duration-300 {{ $performance['is_expired'] ? 'bg-red-500' : 'bg-blue-500' }}"
                                 style="width: {{ min(100, ($performance['days_elapsed'] / $performance['duration_days']) * 100) }}%"></div>
                        </div>
                        @endif
                    </div>

                    {{-- Referral Metrics (Direct + Indirect) --}}
                    <div>
                        <h4 class="text-xs font-bold text-gray-500 uppercase tracking-wider mb-3">
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
                            <div class="bg-green-50 rounded-xl p-3 text-center border border-green-200">
                                ...
                                <p class="text-xl font-extrabold text-green-700">{{ $performance['direct_referrals'] }}</p>
                            </div>
                            <div class="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                                <p class="text-[10px] text-blue-400 font-bold uppercase">Indirect</p>
                                <p class="text-xl font-extrabold text-blue-700">{{ $performance['indirect_referrals'] }}</p>
                            </div>
                            <div class="bg-indigo-50 rounded-xl p-3 text-center border border-indigo-200">
                                ...
                                <p class="text-xl font-extrabold text-indigo-700">{{ $performance['total_referrals'] }}</p>
                            </div>
                            <div class="bg-purple-50 rounded-xl p-3 text-center border border-purple-200">
                                ...
                                <p class="text-xl font-extrabold text-purple-700">{{ $performance['total_active_referrals'] }}</p>
                                <p class="text-[9px] text-purple-400 mt-0.5">{{ $performance['active_referrals'] }} direct + {{ $performance['active_indirect_referrals'] }} indirect</p>
                            </div>
                            <div class="bg-amber-50 rounded-xl p-3 text-center border border-amber-200">
                                ...
                                <p class="text-xl font-extrabold text-amber-700">${{ number_format($performance['referral_bonuses'], 2) }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- SUPER LEADER: Credit Turnover Progress --}}
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                            <div class="bg-yellow-50 rounded-xl p-3 text-center border border-yellow-200">
                                ...
                                <p class="text-sm font-extrabold text-yellow-700">${{ number_format($credit->credit_amount, 2) }}</p>
                            </div>
                            <div class="bg-green-50 rounded-xl p-3 text-center border border-green-200">
                                <p class="text-[10px] text-green-500 font-bold uppercase">Remaining</p>
                                <p class="text-sm font-extrabold text-green-700">${{ number_format($credit->remaining_credit, 2) }}</p>
                            </div>
                            <div class="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                                <p class="text-[10px] text-blue-500 font-bold uppercase">Turnover Target</p>
                                <p class="text-sm font-extrabold text-blue-700">${{ number_format($credit->sales_turnover_target, 2) }}</p>
                            </div>
                            <div class="rounded-xl p-3 text-center border {{ $credit->status === 'active' ? 'bg-green-50 border-green-200' : 'bg-gray-50 border-gray-200' }}">
                                <p class="text-[10px] font-bold uppercase {{ $credit->status === 'active' ? 'text-green-500' : 'text-gray-400' }}">Credit Status</p>
                                <p class="text-sm font-extrabold {{ $credit->status === 'active' ? 'text-green-700' : 'text-gray-600' }}">{{ strtoupper($credit->status) }}</p>
                            </div>
                        </div>
                    </div>
                    @endif
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-6">
                    ...
                    @if($leader->status === 'pending' || $leader->status === 'rejected')
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                            ...
                            {{-- Reject Button --}}
                            <form action="{{ route('admin.team-leaders.reject', $leader->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                    ...
                    @elseif($leader->status === 'suspended')
                        ...
                        <form action="{{ route('admin.team-leaders.reactivate', $leader->id) }}" method="POST">
                            @csrf
                            <button type="submit"
                                    class="flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-6 rounded-xl shadow-sm transition duration-150 text-sm">
    ...
    <div id="approveModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <div class="bg-white p-6 rounded-2xl max-w-lg w-full shadow-2xl border border-gray-100 text-left max-h-[90vh] overflow-y-auto">
            <h4 class="font-bold text-lg text-slate-800 mb-4 border-b pb-2 flex items-center gap-2">
                <i class="fas fa-user-check text-green-600"></i> Approve: {{ $leader->Names }}
                <span class="ml-auto text-xs font-bold px-2.5 py-1 rounded-full {{ ($leader->leadership_level ?? 'TEAM_LEADER') === 'SUPER_LEADER' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800' }}">
                    {{ $leader->leadership_level ?? 'TEAM_LEADER' }}
                </span>
            </h4>
            <form action="{{ route('admin.team-leaders.approve', $leader->id) }}" method="POST" class="space-y-4">
                @csrf
                <input type="hidden" name="leadership_level" value="{{ $leader->leadership_level ?? 'TEAM_LEADER' }}">
    ...
    {{-- ═══════════════════════════════════════════════════════════
         SUPER LEADER CREDIT MANAGEMENT (after approval)
         ═══════════════════════════════════════════════════════════ --}}
    @if(($leader->leadership_level ?? 'TEAM_LEADER') === 'SUPER_LEADER' && $leader->status === 'confirmed')
        @php $credit = $leader->superLeaderCredit; @endphp
        <div id="credit-panel" class="bg-white rounded-2xl shadow-sm border border-yellow-200 overflow-hidden mt-6">
            <div class="px-6 py-4 border-b border-yellow-100 bg-yellow-50">
                ...
                <p class="text-xs text-yellow-600 mt-1">Configure credit conditions for {{ $leader->Names }}. Changes take effect immediately.</p>
            </div>
            <div class="p-6">
                @if($credit)
                <form action="{{ route('admin.team-leaders.credit.update', $credit->id) }}" method="POST" class="space-y-5">
                    @csrf
                    ...
                    <div class="grid grid-cols-3 gap-4 mb-4">
                        <div class="bg-gray-50 rounded-xl p-3 text-center border">
                            ...
                            <p class="text-lg font-extrabold text-slate-800">${{ number_format($credit->credit_amount, 2) }}</p>
                        </div>
                        <div class="bg-green-50 rounded-xl p-3 text-center border border-green-200">
                            <p class="text-xs text-green-500 font-bold uppercase">Remaining</p>
                            <p class="text-lg font-extrabold text-green-700">${{ number_format($credit->remaining_credit, 2) }}</p>
                        </div>
                        <div class="bg-blue-50 rounded-xl p-3 text-center border border-blue-200">
                            <p class="text-xs text-blue-500 font-bold uppercase">Cashout</p>
                            <p class="text-lg font-extrabold text-blue-700">${{ number_format($credit->cashout_amount, 2) }}</p>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" name="sales_turnover_target" min="0" step="0.01" value="{{ $credit->sales_turnover_target }}" class="w-full bg-gray-50 border border-gray-300 text-slate-900 rounded-xl p-3 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-bold">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="turnover_target_percent" min="0" max="100" step="0.01" value="{{ $credit->turnover_target_percent }}" class="w-full bg-gray-50 border border-gray-300 text-slate-900 rounded-xl p-3 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-bold">
                        ...
                        <div>
                            ...
                            <input type="number" name="turnover_reward_percent" min="0" max="100" step="0.01" value="{{ $credit->turnover_reward_percent }}" class="w-full bg-gray-50 border border-gray-300 text-slate-900 rounded-xl p-3 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-bold">
                    ...
                    <div>
                        ...
                        <input type="number" name="auto_withdrawal_percent" min="0" max="100" step="0.01" value="{{ $credit->auto_withdrawal_percent }}" class="w-full bg-gray-50 border border-gray-300 text-slate-900 rounded-xl p-3 text-sm focus:ring-yellow-500 focus:border-yellow-500 font-bold">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex gap-3">
                                <button type="submit" name="credit_status" value="active"
                                        class="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 text-white font-bold py-2.5 px-4 rounded-xl text-sm transition duration-150 {{ $credit->status === 'active' ? 'ring-2 ring-green-400 ring-offset-2' : '' }}">
                                    <i class="fas fa-check-circle"></i> Active
                                </button>
                                <button type="submit" name="credit_status" value="pending"
                                        class="flex-1 flex items-center justify-center gap-2 bg-gray-400 hover:bg-gray-500 text-white font-bold py-2.5 px-4 rounded-xl text-sm transition duration-150 {{ $credit->status === 'pending' ? 'ring-2 ring-gray-400 ring-offset-2' : '' }}">
                ...
                @else
                    {{-- No credit record yet — show create form --}}
                    ...
                    <form action="{{ route('admin.team-leaders.credit.create', $leader->id) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs font-bold text-gray-700 uppercase mb-1">Credit Amount ($) <span class="text-red-500">*</span></label>
