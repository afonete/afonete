...
<div class="container bg-white min-h-screen py-4 px-3">
    <div class="mb-3">
        <a href="{{ route('admin.contracts.index') }}" class="text-blue-600 hover:text-blue-800 text-sm">← Back to Client Contracts</a>
    </div>

    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        <div>
            <h1 class="text-2xl uppercase text-slate-700 font-bold">Client Contract Details</h1>
            <p class="text-gray-500 text-sm">Bifonex Contract #{{ $contract->id }}</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        <div class="border border-gray-200 rounded-xl shadow-sm p-4 bg-slate-50">
            ...
            <div class="text-lg font-bold text-gray-800">{{ $contract->user->name ?? $contract->name }}</div>
        ...
        <div class="border border-gray-200 rounded-xl shadow-sm p-4 bg-slate-50">
            ...
            <div class="text-sm text-gray-800">Username: <strong>@ {{ $contract->user->user ?? '—' }}</strong></div>
            <div class="text-sm text-gray-800">User ID: <strong>#{{ $contract->user_id }}</strong></div>
            <div class="text-sm text-gray-800">Country: <strong>{{ $contract->user->country ?? '—' }}</strong></div>
        </div>
        <div class="border border-gray-200 rounded-xl shadow-sm p-4 bg-slate-50">
            ...
            <div class="text-xs text-gray-500">Signed: {{ $contract->created_at ? $contract->created_at->format('Y-m-d H:i:s') : '—' }}</div>
            <div class="text-xs text-gray-400 font-mono mt-0.5">Ref: {{ $contract->contract }}</div>
        </div>
    </div>
    ...
    <div class="border border-gray-200 rounded-xl shadow-md overflow-hidden bg-slate-900">
        <div class="px-5 py-3.5 bg-slate-900 text-white font-bold text-sm flex items-center justify-between border-b border-slate-800">
            ...
            <span class="text-xs text-slate-400 font-mono">System Ref #{{ $contract->id }}</span>
        </div>
        <iframe
