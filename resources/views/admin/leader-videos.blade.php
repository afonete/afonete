...
<div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8 max-w-7xl">
    ...
    <header class="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-100 py-6 px-6 rounded-2xl mb-8 shadow-sm">
        <div class="flex items-center justify-between flex-wrap gap-4">
            ...
            <a href="{{ route('admin.leader-videos.upload') }}" class="inline-flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 px-5 rounded-xl shadow-sm transition duration-150 text-sm">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-xl p-4 border shadow-sm text-center">
            ...
            <p class="text-2xl font-extrabold text-slate-800">{{ $allVideos->count() }}</p>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 bg-white">
                        @foreach($allVideos as $v)
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-3">
                                    @if($v->video_url && ($v->video_type === 'youtube'))
                                ...
                                <td class="px-6 py-3 font-semibold text-gray-800 max-w-[200px] truncate">{{ $v->title }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ number_format($v->views_count ?? 0) }}</td>
                                <td class="px-6 py-3 text-gray-600">{{ $v->duration_seconds ?? $v->targeted_duration ?? '—' }}s</td>
                                <td class="px-6 py-3">
                                ...
                                <td class="px-6 py-3 text-gray-500 text-xs">{{ $v->created_at->format('d M Y') }}</td>
                                <td class="px-6 py-3 text-right space-x-2">
                                    @if($v->status !== 'approved' && $v->status !== 'active')
                                        <a href="{{ route('admin.leader-videos.approve', $v->id) }}" class="inline-flex items-center gap-1 bg-green-600 hover:bg-green-700 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition">
                                            <i class="fas fa-check"></i> Approve
                                        </a>
                                    @endif
                                    @if($v->status !== 'rejected')
                                        <button type="button" onclick="document.getElementById('reject-{{ $v->id }}').classList.remove('hidden')" class="inline-flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition">
                                            <i class="fas fa-times"></i> Reject
                                        </button>
                                    @endif
                                    <a href="{{ route('admin.leader-videos.delete', $v->id) }}" onclick="return confirm('Delete this video permanently?')" class="inline-flex items-center gap-1 bg-gray-500 hover:bg-gray-600 text-white font-bold py-1.5 px-3 rounded-lg text-xs transition">
                                        <i class="fas fa-trash"></i>
                                    </a>
                                </td>
                            </tr>
                            {{-- Reject Modal --}}
                            <tr id="reject-{{ $v->id }}" class="hidden bg-red-50">
                                <td colspan="7" class="px-6 py-4">
                                    <form action="{{ route('admin.leader-videos.reject', $v->id) }}" method="POST" class="flex items-center gap-3">
                                        @csrf
                                        ...
                                        <button type="button" onclick="document.getElementById('reject-{{ $v->id }}').classList.add('hidden')" class="bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold py-2 px-4 rounded-xl text-xs">Cancel</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
