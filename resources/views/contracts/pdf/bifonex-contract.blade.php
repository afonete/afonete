...
<table class="meta">
    <tr>
        ...
        <td>{{ $user->name ?? $contract->name }}</td>
    </tr>
    <tr>
        <td class="label">Username / ID</td>
        <td>{{ $user->user ?? '—' }} / #{{ $user->id ?? $contract->user_id }}</td>
    </tr>
    <tr>
...
<div class="signature-section">
    ...
    <div class="signature-box">
        @if($signatureDataUri)
            <img src="{{ $signatureDataUri }}" alt="Client signature">
        @else
            <span class="small">Signature image not found.</span>
        @endif
    </div>
    <div class="signature-line">
        {{ $user->name ?? $contract->name }}<br>
        <span class="small">Signed on {{ $signedAt }}</span>
    </div>
</div>

<div class="footer">Bifonex Contract — Generated by the system. Contract ID #{{ $contract->id }}</div>
</body>
</html>
