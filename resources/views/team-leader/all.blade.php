<div class="wrapper">
    ...
    <div class="all-page">
        ...
        <div class="page-grid">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="credit-box">
                                ...
                                <div class="row-stat"><span>Total</span><span class="val">${{ number_format($credit->credit_amount,2) }}</span></div>
                                <div class="row-stat"><span>Remaining</span><span class="val">${{ number_format($credit->remaining_credit,2) }}</span></div>
                                <div class="row-stat"><span>Cashout</span><span class="val">${{ number_format($credit->cashout_amount,2) }}</span></div>
                                <div class="row-stat"><span>Status</span><span class="val" style="color:{{ $credit->status==='active'?'#34d399':'#fbbf24' }}">{{ strtoupper($credit->status) }}</span></div>
                            </div>
                            @endif
                            <div class="zoom-cta">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="task-block">
                                ...
                                @if($tasks)<div style="white-space:pre-line">{{ $tasks }}</div>@else<p style="opacity:.6">No tasks assigned.</p>@endif
                            </div>
                        ...
                        <div class="card-body">
                            <div class="events-layout">
                                <div class="history-panel">
                                    ...
                                    <div id="history-plan">@forelse($planEvents as $e)<div class="eh-item"><strong>{{ $e->title }}</strong> <span class="eh-badge {{ $e->status }}">{{ $e->status }}</span></div>@empty<div class="eh-empty">No event plan history.</div>@endforelse</div>
                                    <div id="history-zoom" style="display:none">@forelse($zoomEvents as $e)<div class="eh-item"><strong>{{ $e->title }}</strong> <span class="eh-badge {{ $e->status }}">{{ $e->status }}</span></div>@empty<div class="eh-empty">No zoom history.</div>@endforelse</div>
                                </div>
                                <div class="form-panel">
                                    <h4>Report Event</h4>
                                    <form action="{{ route('team-leader.event-report') }}" method="POST" enctype="multipart/form-data">
                                        @csrf<input type="hidden" name="event_type" id="form-type" value="plan">
                            ...
                            <div class="carousel-strip" id="carousel">@foreach($eventImages as $img)<img src="{{ asset('storage/'.$img) }}" alt="">@endforeach @if($eventImages->isEmpty())<span style="color:#9ca3af;font-size:.7rem;padding:12px">No event photos yet.</span>@endif</div>
                ...
                <div class="row row-2">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <div class="amb-cards">
                                @foreach(['Instagram'=>'Promote Bifonex through posts and reels.','Twitter'=>'Promote through Retweets, Tweets, and Threads.','TikTok'=>'Promote Bifonex through short-form videos.','YouTube'=>'Promote through long-form video content.','Support'=>'Provide support via likes, comments and reposts.','Creator'=>'Create original content promoting Bifonex.'] as $p => $d)
                                    <div class="amb-item"><h5>{{ $p }}</h5><p>{{ $d }}</p><button onclick="openModal('{{ $p }}')">Apply now</button></div>
                                @endforeach
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            @if($adminVideos->isNotEmpty())
                                ...
                                <div class="video-hero" id="vMain">@if($fv->video_type==='youtube'&&$fv->youtubeId())<iframe src="https://www.youtube.com/embed/{{ $fv->youtubeId() }}" allowfullscreen></iframe>@elseif($fv->video_url)<video controls><source src="{{ asset('storage/'.$fv->video_url) }}" type="video/mp4"></video>@endif</div>
                                <div class="video-strip">@foreach($adminVideos as $i=>$v)<div class="v-thumb {{$i===0?'active':''}}" onclick="playV(this,'{{$v->video_type}}','{{$v->video_type==='youtube'?$v->youtubeId():asset('storage/'.$v->video_url)}}')">@if($v->video_type==='youtube'&&$v->youtubeId())<img src="https://img.youtube.com/vi/{{$v->youtubeId()}}/mqdefault.jpg" alt="">@else<div style="display:flex;align-items:center;justify-content:center;height:100%;color:#64748b;font-size:.55rem;padding:4px;text-align:center">{{$v->title}}</div>@endif</div>@endforeach</div>
                            @else<div style="background:#f9fafb;border-radius:8px;padding:40px;text-align:center;color:#9ca3af;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center"><i class="fas fa-video fa-2x" style="margin-bottom:8px"></i><span style="font-size:.75rem">No videos available</span></div>@endif
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="sidebar-ads">
                @forelse($adminBanners as $b)<div class="ad-slot">@if($b->image_path)<img src="{{ asset('storage/'.$b->image_path) }}" alt="{{$b->title}}">@else<span class="empty">{{$b->title}}</span>@endif</div>@empty
                <div class="ad-slot"><span class="empty">Ad Space</span></div><div class="ad-slot"><span class="empty">Ad Space</span></div><div class="ad-slot"><span class="empty">Ad Space</span></div><div class="ad-slot"><span class="empty">Ad Space</span></div><div class="ad-slot"><span class="empty">Ad Space</span></div><div class="ad-slot"><span class="empty">Ad Space</span></div>@endforelse
            </div>
        </div>
    </div>

    {{-- MODAL --}}
    <div class="modal-overlay" id="ambModal"><div class="modal-box"><h4>Apply for Ambassador Program</h4><form action="{{ route('team-leader.socials.store') }}" method="POST">@csrf<label>Platform</label><select name="platform" id="mPlat"><option>Instagram</option><option>Twitter</option><option>TikTok</option><option>YouTube</option><option>Support</option><option>Creator</option></select><label>Follower Count</label><input type="number" name="views" min="0" value="0"><label>Profile URL</label><input type="url" name="profile_link" required placeholder="https://..."><div class="modal-actions"><button type="button" class="cancel" onclick="closeModal()">Cancel</button><button type="submit" class="confirm">Submit Application</button></div></form></div></div>

    <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
    <script>
    // Countdown
    !function(){@if($expiryDate)var e=new Date('{{$expiryDate->toIso8601String()}}').getTime();@else var e=Date.now()+60*864e5;@endif
    ...
    // Charts
    document.addEventListener('DOMContentLoaded',function(){var a=document.getElementById('lineChart');if(a)new Chart(a,{type:'line',data:{labels:['W1','W2','W3','W4','W5','W6'],datasets:[{data:[{{rand(0,max(1,$directReferrals))}},{{rand(0,max(1,$directReferrals))}},{{$directReferrals}},{{$activeReferrals}},{{$directReferrals}},{{$activeReferrals}}],borderColor:'#6366f1',backgroundColor:'rgba(99,102,241,.08)',fill:true,tension:.4,pointRadius:2,borderWidth:2}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false}},scales:{y:{beginAtZero:true,grid:{color:'#f3f4f6'},ticks:{font:{size:8}}},x:{grid:{display:false},ticks:{font:{size:8}}}}}});var b=document.getElementById('donutChart');if(b)new Chart(b,{type:'doughnut',data:{labels:['Active','Inactive'],datasets:[{data:[{{$activeReferrals}},{{max(0,$directReferrals-$activeReferrals)}}],backgroundColor:['#6366f1','#e5e7eb'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,cutout:'60%',plugins:{legend:{position:'bottom',labels:{font:{size:8},padding:6}}}}})});
    </script>
</div>
